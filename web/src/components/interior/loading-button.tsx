/**
 * Vendored from interior.dev — https://www.interior.dev/docs/loading-button
 * by Ozzy (https://x.com/ozzyxs1a), MIT licensed. Motion logic unchanged.
...
export function useAsyncAction({
  ...
  resetAfter = 1400,
  onError,
}: UseAsyncActionOptions = {} as UseAsyncActionOptions) {
  ...
  const act = useRef<(() => unknown) | undefined>(action);
  const fail = useRef<((error: unknown) => void) | undefined>(onError);
  ...
  return (
    <>
      <motion.button
        ...
        className={`pressable relative inline-flex h-11 select-none items-center justify-center gap-3 rounded-full border-2 border-coal bg-coal px-7 font-mono text-xs tracking-[0.25em] text-cream outline-none transition-[border-color,box-shadow,background-color] duration-150 hover:bg-black focus-visible:border-ember disabled:opacity-50 ${className}`}

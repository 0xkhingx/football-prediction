/**
 * Vendored from interior.dev — https://www.interior.dev/docs/accordion
 * by Ozzy (https://x.com/ozzyxs1a), MIT licensed. Motion/a11y logic unchanged.
 * Surfaces restyled to the Matchday Fate token system (see RESTYLE notes).
 */
"use client";
...
export function Accordion({
  ...
  return (
    <div
      className={`divide-y divide-coal/10 overflow-hidden rounded-[11px] border border-coal/15 bg-cream ${className}`}
    >
      {items.map((item) => (
        <AccordionRow
          key={item.id}
    ...
    <div>
      <div role="heading" aria-level={headingLevel}>
        ...
        >
          ...
          {item.meta ? (
            <span className="shrink-0 font-mono text-[11.5px] tabular-nums text-coal/50">
              {item.meta}
            </span>
          ) : null}
      ...
      >
        <div
          {...panel}
          // React 18.3 compat: useRef(null) object refs aren't assignable to
          // JSX ref props under the backported 19-style overloads.
          ref={ref as unknown as React.LegacyRef<HTMLDivElement>}
          className="border-t border-coal/10 bg-white/70"
          style={{
        ...
        >
          <motion.div
            ...
            className="px-3.5 pb-3.5 pt-3 text-[12.5px] leading-relaxed text-coal/70"
          >
            {item.content}
          </motion.div>
        </div>
